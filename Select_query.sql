...
from 
	track
where 
	duration >=240 order by duration;
...
from 
	compilation 
where 
	year_compilation BETWEEN 2018 and 2024;
...
from 
	track  
where
	string_to_array(lower(name_track),' ') && string_to_array('мой my моя',' '); 
...
where 
	a.year_album between 2019 and 2024;
...
select 
	a.name_album,
	avg(t.duration) average_durations,
	count(t.id_track) count_tracks,
	sum(t.duration) as sum_of_tracks 
from
	track t 
join album a 
on t.id_album = a.id_album 
...
from 
	compilation C
join
compltrack CT on C.id_compilation =CT.id_compilation 
join
	track T 
on CT.id_track = T.id_track 
join
	track TR 
on CT.id_track=TR.id_track
join
	album AL 
on tr.id_album = AL.id_album 
join
	artistalbum AA 
on AL.id_album = AA.id_album 
join
	artist ART 
on AA.id_artist = ART.id_artist 
	where lower(ART.name_artist)  like '%girls%' 
group by 
	C.id_compilation,
	art.id_artist; 
...
left join 
	compltrack c 
on t.id_track = c.id_track  
where 
	id_compilation is null order by t.id_track;
